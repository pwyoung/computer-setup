    ...
    prompt(actionContext, ui) {
        return __awaiter(this, void 0, void 0, function* () {
            for (const step of this._promptSteps) {
                ...
                this._wizardContext = yield step.prompt(this._wizardContext, ui);
                if (step.subWizard) {
                    step.subWizard._parentWizard = this;
                    this._subWizards.push(step.subWizard);
                    yield step.subWizard.prompt(actionContext, ui);
                }
            }
            return this._wizardContext;
        });
    }
    execute(actionContext, outputChannel) {
        return __awaiter(this, void 0, void 0, function* () {
            outputChannel.show(true);
            for (const subWizard of this._subWizards) {
                yield subWizard.execute(actionContext, outputChannel);
            }
            for (const step of this._executeSteps) {
                actionContext.properties.lastStepAttempted = `execute-${step.constructor.name}`;
                this._wizardContext = yield step.execute(this._wizardContext, outputChannel);
            }
            return this._wizardContext;
        });
    }
