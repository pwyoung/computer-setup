...
<div class="fullsize vertBox">
    ...
    <div id="messages" class="scrollable messages" [class.hidden]="!messageActive && dataSets.length !== 0"
        ...
        <table id="messageTable">
            ...
            <tbody>
            <tr class='messagePreview'>
                <td nowrap><span *ngIf="previewComplete">{{Utils.showPreviewMessage(rows)}}</span></td>
            </tr>
                <template ngFor let-message [ngForOf]="messages">
                    <tr class='messageRow'>
                        <td><span *ngIf="!queryCancelComplete && !Utils.isNumber(message.batchId) && !previewComplete">[{{message.time}}]</span></td>
                ...
                <tr id='executionSpinner' *ngIf="!queryCancelComplete && !complete  && !previewComplete">
                ...
                <tr *ngIf="queryCancelComplete">
                    <td></td>
                    <td>{{Utils.formatString(Constants.queryCancelCompleteMessage)}}</td>
                </tr>
            </tbody>
        </table>
    </div>
...
let AppComponent = class AppComponent {
    constructor(dataService, shortcuts, _el, cd) {
        ...
        this.queryCancelComplete = false;
        /**
        ...
        this.dataService.dataEventObs.subscribe(event => {
            switch (event.type) {
                ...
                case 'previewComplete':
                    self.rows = event.data;
                    self.previewComplete = true;
                    break;
                case 'queryCancelComplete':
                    self.queryCancelComplete = true;
                    self.messagesAdded = true;
                    break;
                case 'message':
