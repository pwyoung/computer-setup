...
const hiveExecute_1 = require("../models/contracts/hiveExecute");
class QueryNotificationHandler {
    ...
    initialize() {
        serviceclient_1.HiveToolsServiceClient.Instance.onNotification(queryExecute_1.QueryExecuteCompleteNotification.type, this.handleQueryCompleteNotification());
        serviceclient_1.HiveToolsServiceClient.Instance.onNotification(queryExecute_1.QueryExecuteBatchStartNotification.type, this.handleBatchStartNotification());
        serviceclient_1.HiveToolsServiceClient.Instance.onNotification(queryExecute_1.QueryExecuteBatchCompleteNotification.type, this.handleBatchCompleteNotification());
        serviceclient_1.HiveToolsServiceClient.Instance.onNotification(queryExecute_1.QueryExecuteResultSetCompleteNotification.type, this.handleResultSetCompleteNotification());
        serviceclient_1.HiveToolsServiceClient.Instance.onNotification(queryExecute_1.QueryExecuteMessageNotification.type, this.handleMessageNotification());
        serviceclient_1.HiveToolsServiceClient.Instance.onNotification(hiveExecute_1.HiveYarnIdAppFoundNotification.type, this.handleHiveYarnIdFoundNotification());
        serviceclient_1.HiveToolsServiceClient.Instance.onNotification(hiveExecute_1.HiveYarnLogNotification.type, this.handleHiveYarnLogNotification());
    }
    handleHiveYarnLogNotification() {
        const self = this;
        return (event) => {
            let handlerCallback = (runner) => {
                runner.handleHiveYarnLog(event);
            };
            self.enqueueOrRun(handlerCallback, self._queryRunners.get(event.ownerUri));
        };
    }
    handleHiveYarnIdFoundNotification() {
        const self = this;
        return (event) => {
            let handlerCallback = (runner) => {
                runner.handleHiveYarnAppIdFound(event);
            };
            self.enqueueOrRun(handlerCallback, self._queryRunners.get(event.ownerUri));
        };
    }
    ...
    handleQueryCompleteNotification() {
        ...
        return (event) => {
            let handlerCallback = (runner) => {
                runner.handleQueryComplete(event, runner.isPreview);
