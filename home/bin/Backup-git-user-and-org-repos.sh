...
if [ -z "$GITHUB_TOKEN" ]; then echo "token is not set" && exit 1; fi
if [ -z "$GITHUB_USER" ]; then echo "user is not set" && exit 1; fi
#if [ -n "$GITHUB_ORG" ]; then echo "org is set"; else echo "org is not set" && exit 1; fi

if [ -z "$GITHUB_REPO_LIST" ]; then echo "repo_list is not set" && exit 1; fi
if [ -z "$GITHUB_TEMP_FILE" ]; then echo "temp_file is not set" && exit 1; fi
...
clone_org_repos() {
    echo "Cloning repos for orgs: ${GITHUB_ORGS}"
